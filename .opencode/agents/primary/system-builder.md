...
1. **Orient.** Understand the mandate, the OpenCode harness constraints, and any existing artefacts. Read the authoritative convention — the OpenCode agent frontmatter schema (see `wiki/harness/agent-frontmatter.md`) — before assuming; do not re-read your own prompt — it is already in context and is itself the example.
