...
9. **Runtime artifact, not document.** An agent file is a prompt the harness loads, not a navigable document. The body is text fed to a model whose filesystem references resolve against the runtime session CWD, never the file's directory. Therefore the body must not contain filesystem paths or hyperlinks to other files; reference resources by name and let the agent resolve them via `glob`/`read` at runtime. The `## Related` link idiom belongs to rendered documents (wiki pages), not agent prompts. *Why:* a path or link in a prompt is dead weight at best and a runtime error at worst — the model cannot click it, and any path it acts on resolves against a CWD the file does not control.
...
- **The self-check.**
  ...
  - *Permission correctness* — are high-impact actions gated via frontmatter `permission`, not merely advised in prose?
   ...
   - *Runtime artifact* — does the body avoid filesystem paths and hyperlinks, referencing resources by name for resolution via `glob`/`read` at runtime?
...
| Document idiom in a prompt | `## Related` links, relative paths, and hyperlinks embed wiki/document conventions in a runtime artifact; the model cannot resolve them and they break once the file is installed elsewhere | Treat the agent file as a runtime prompt; reference resources by name and resolve via `glob`/`read` |
