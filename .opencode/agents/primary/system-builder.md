...
9. **Runtime artifact, not document.** An agent file is a prompt the harness loads, not a navigable document. The body is text fed to a model whose filesystem references resolve against the runtime session CWD, never the file's directory. **Reference resources by name and let the agent resolve them via `glob`/`read` at runtime** — the body must not embed *reference* paths or hyperlinks to other files (a safety-critical boundary: such a path resolves against a CWD the file does not control and is a runtime error). **Exclusion — action-target paths:** a body *may* name paths relative to the runtime CWD when they are targets the agent itself creates or operates on within a workflow (e.g. "make `.tmp/` and curl the file into it"). These are execution instructions, not resource references; they resolve correctly because the agent acts in the session CWD. The prohibition covers *reference* paths (to read), not *target* paths (to act on). The `## Related` link idiom belongs to rendered documents (wiki pages), not agent prompts. *Why:* a reference path or link in a prompt is dead weight at best and a runtime error at worst — the model cannot click it, and any path it reads resolves against a CWD the file does not control.
...
- **The self-check.**
  ...
  - *Permission correctness* — are high-impact actions gated via frontmatter `permission`, not merely advised in prose?
   ...
   - *Runtime artifact* — does the body reference resources by name for resolution via `glob`/`read` at runtime, avoiding *reference* paths and hyperlinks while permitting *action-target* paths relative to the runtime CWD that the agent creates or operates on within a workflow?
...
| Document idiom in a prompt | `## Related` links and *reference* paths/hyperlinks embed wiki/document conventions in a runtime artifact; the model cannot resolve them and they break once installed elsewhere. (Action-target paths relative to the runtime CWD — e.g. a workflow writing to `.tmp/` — are exempt, as the agent resolves them at execution.) | Treat the agent file as a runtime prompt; reference resources by name and resolve via `glob`/`read`; permit only action-target paths the agent itself creates or operates on |
