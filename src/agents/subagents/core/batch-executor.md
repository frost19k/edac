...
On a subtask failure, report it to the orchestrator immediately (fail-fast), then complete this agent's own sequence as the orchestrator directs.
...
**TaskManager creates**:
- Task 01: Design UI components (parallel: true, agent: FrontendSpecialist)
...
```
Batch 1: 
  - Delegate to FrontendSpecialist (Task 01)
...
- **Batch atomicity**: Entire batch must complete before proceeding (except: fail-fast subtask reporting to orchestrator is permitted immediately, even if other subtasks are still running)
