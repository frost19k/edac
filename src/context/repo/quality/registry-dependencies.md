...
dependencies:
  ...
  - subagent:test-engineer      # Can delegate to test-engineer
...
dependencies:
  ...
  - context:repo/guides/adding-agent  # Project-specific context
```
...
✗ Dependency not found: open-coder → context:core/standards/code

Missing dependencies: 1
  - open-coder (agent) → context:core/standards/code
...
```bash
git commit -m "Update code standards - affects open-coder, open-agent, code-reviewer"
```
...
**3. Delete component**:
```bash
rm agents/subagents/example-agent.md
```
...
/check-context-deps reports:
  open-coder: missing context:core/standards/code
```
...
# Option 2: Manual fix
# Edit .opencode/agents/core/open-coder.md
# Add to frontmatter:
dependencies:
  - context:core/standards/code
...
- **Registry Guide**: the registry guide
- **Registry Concepts**: the registry concepts
- **Adding Agents**: `.opencode/context/repo/guides/adding-agent-basics.md`
