...
tags: [context, onboarding, intelligence, wizard]
dependencies:
...
<critical_rules priority="absolute" enforcement="strict">
  <rule id="project_intelligence">
    MUST create technical-domain.md in intelligence/ dir (NOT single project-context.md)
  </rule>
...
<execution_priority>
  <tier level="1" desc="Project Intelligence + MVI + Standards">
    - @project_intelligence (technical-domain.md in intelligence/ dir)
...
**What happens**:
1. Saves to `.opencode/context/intelligence/` in your project (always local)
...
**Default behavior**: Always use local `.opencode/context/intelligence/`.
**Override**: `--global` flag saves to `~/.config/opencode/context/intelligence/` instead.

**Resolution:**
1. If `--global` flag → `$CONTEXT_DIR = ~/.config/opencode/context/intelligence/`
2. Otherwise → `$CONTEXT_DIR = .opencode/context/intelligence/` (always local)
...
Check: `$CONTEXT_DIR` (set in Stage 0.5 — either `.opencode/context/intelligence/` or `~/.config/opencode/context/intelligence/`)
...
Will BACKUP existing files to:
  .tmp/backup/intelligence-{timestamp}/
...
Will create:
  - intelligence/technical-domain.md (tech stack & patterns)
  - intelligence/navigation.md (quick overview)
...
<!-- Context: intelligence/technical | Priority: critical | Version: 1.0 | Updated: 2026-01-29 -->
...
Want the same patterns across ALL your projects?
  /add-context --global
  → Saves to ~/.config/opencode/context/intelligence/
...
- Context System: .opencode/context/navigation.md
```
...
frontmatter:
  context: intelligence/technical
...
**System Integration**:
- [ ] @project_intelligence (technical-domain.md in intelligence/)
...
A: `cat .opencode/context/intelligence/technical-domain.md` (local) or `cat ~/.config/opencode/context/intelligence/technical-domain.md` (global)

**Q: Multiple context files?**
A: Yes! Create in your intelligence directory. Agents load all.

**Q: Remove pattern?**
A: Edit directly: `nano .opencode/context/intelligence/technical-domain.md`

**Q: Share w/ team?**
A: Yes! Use local install (`.opencode/context/intelligence/`) and commit to repo. Team members get your patterns automatically.
...
A: Run `/add-context` (defaults to local). Creates `.opencode/context/intelligence/` in your project even if OAC was installed globally.
