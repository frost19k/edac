...
function validateComponentPaths(
  ...
  for (const component of components) {
    ...
    const fullPath = join(REPO_ROOT, MIRROR_DIR, path);
...
function resolveDepStatus(dep: string, componentMap: Map<string, ComponentRef>): string {
  const parsed = parseDependency(dep);
  if (!parsed) return 'invalid_format';
  if (!resolveCategory(parsed.type)) return 'unknown_type';
  if (componentExists(dep, componentMap)) return 'found';

  // Extra path-based lookup for context deps (preserves original behavior)
  if (parsed.type === 'context' && !parsed.id.includes('*')) {
    const contextPath = `context/${parsed.id}.md`;
    for (const component of componentMap.values()) {
      if (component.path === contextPath) return 'found';
    }
  }
  ...
  for (const compType of componentTypes) {
    ...
    for (const component of components) {
      ...
      for (const dep of dependencies) {
        ...
        const result = resolveDepStatus(dep, componentMap);
...
function scanForOrphanedFiles(registry: Registry): void {
  ...
  for (const category of Object.keys(registry.components)) {
    ...
    if (components) {
      components.forEach(c => registryPaths.add(join(MIRROR_DIR, c.path)));
    }
  }
  
  scanDirectory(join(REPO_ROOT, MIRROR_DIR), registryPaths);
}
...
function main(): void {
  ...
  // Scan for orphaned files (informational, does not affect exit code)
