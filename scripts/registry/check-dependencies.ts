...
const CRITICAL_FILES = [
  { id: 'root-navigation', path: 'context/navigation.md', description: 'Root navigation file - ContextScout starts here' },
  { id: 'context-paths-config', path: 'context/core/config/paths.json', description: 'Context paths configuration - loaded via @ reference' },
  { id: 'context-system', path: 'context/core/context-system.md', description: 'Context system guide' },
];
...
// Load profiles from registry.json
function loadProfiles(): Record<string, { name: string; description: string; components: string[] }> {
...
function main(): void {
  ...
  // Count real component entries (map is double-keyed: type:id + bare id, so don't use map.size)
  const categories = ['agents', 'subagents', 'commands', 'tools', 'plugins', 'contexts', 'config', 'skills'];
  const realCount = categories.reduce((n, cat) => {
    const arr = registry.components[cat as keyof Registry['components']];
    return n + (arr?.length ?? 0);
  }, 0);
  printInfo(`Loaded ${realCount} components`);
  console.log('');
