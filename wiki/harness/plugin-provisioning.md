...
**Injected awareness.** The plugin injects a baseline system prompt (`MEMORY_SYSTEM_PROMPT`) into every agent's system context via the `experimental.chat.system.transform` hook — this is the awareness layer that all agents receive, including non-EDAC agents on the same harness. The prompt teaches *when to store* (a hard durability gate: "store only if a future session would need it; otherwise, omit"), not *what the API is* (the tool descriptions handle that). It includes a Store/Skip boundary (durable facts vs. transient session info) with a compress cross-reference (transient context routes to `compress`, not `fact_store`), category descriptions (excluding "state" and "platform facts" as ingress paths for transient info), entity/tag guidance for HRR-compatible fact structuring, and a pointer to the holographic-memory skill for advanced fact-structuring and retrieval-strategy guidance. Agent body text builds on this baseline — see [Tool Awareness Tiers](../framework/tool-awareness-tiers.md) for the per-agent tier matrix (holo-mem column).
...
3. **Agent guidance** — the injected prompt's hard durability gate and compress cross-reference prevent transient info from entering the store; `fact_feedback: unhelpful` sinks low-quality facts via the -0.10/2× trust asymmetry.

The plugin is built from source at install time; its config (`holographic_memory.json`) and skill (`holographic-memory/SKILL.md`) are registered as standalone EDAC components at canonical `src/` locations, not nested inside the plugin directory. The skill is a non-redundant on-demand reference — it carries HRR write-side hygiene (entity specificity, fan-out crosstalk, self-containment, stable vocabulary) with worked good/bad examples and a retrieval-selection guide (query-shape → action), not the tool API (the injected prompt and tool descriptions already cover that). It is loaded via the injected prompt's pointer or OpenCoder's body route; its frontmatter `description` is a "when to load" routing key, not a tool-API restatement.
