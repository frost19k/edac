...
- [Plugin Provisioning](harness/plugin-provisioning.md) — the 4 plugins (DCP, Vibeguard, PTY, Holographic-memory): auto-managed vs per-agent awareness, injected baseline prompt with hard durability gate, compress cross-reference, skill pointer, three-tier pruning, repositioned skill as on-demand reference, configs, build-at-install.
