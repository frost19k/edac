...
> **Plugin-injected baseline.** For holographic-memory, the plugin injects a baseline system prompt (`MEMORY_SYSTEM_PROMPT`) into every agent's system context via `experimental.chat.system.transform` — this is the awareness floor that all agents receive, including non-EDAC agents on the same harness. The prompt includes a pointer to the holographic-memory skill for advanced fact-structuring and retrieval-strategy guidance; the skill is an on-demand reference loaded via this pointer or via OpenCoder's body route. The body-text tiers (below) build on top of this baseline. See [Plugin Provisioning](../harness/plugin-provisioning.md) § Holographic-memory.
