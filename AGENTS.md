...
- `registry.json` lives at the **repo root**, not in `src/`. It is the sole source of truth for components, dependencies, and the Developer profile seed (`profiles.developer.components`, 32 entries) that `install.sh` actually installs.
